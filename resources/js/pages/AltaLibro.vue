<template>
  <div class="row">
    <div class="col-lg-8 m-auto">
      ...
      <card v-else :title="$t('Alta Libro')">
        <form @submit.prevent="register" @keydown="form.onKeydown($event)">
          <!-- Author -->
          <div class="form-group row">
            <label class="col-md-3 col-form-label text-md-right">{{ $t('author') }}</label>
            <div class="col-md-7">
              <input v-model="form.author" :class="{ 'is-invalid': form.errors.has('author') }" class="form-control" type="text" name="author">
          ...
          <div class="form-group row">
            ...
            <!-- descripcion -->
          <div class="form-group row">
            <label class="col-md-3 col-form-label text-md-right">{{ $t('imagen') }}</label>
            <div class="col-md-7">
              <input @change='selectFile'   type="file" name="imagen">
              <has-error :form="form" field="imagen" />
            </div>
          </div>
...
export default {
  ...
  methods: {
    async register () {
      ...
      const { data } = await this.form.post('api/crear/libro')
